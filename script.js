...
function dijkstra(start, end) {
  if (!start || !end || isImpassable(world[start.row][start.col]) || isImpassable(world[end.row][end.col])) {
    return { path: null, explored: [], nodesExplored: 0 };
  }
  ...
  while (openSet.length > 0) {
    ...
    for (const next of neighbors(current)) {
      ...
      const tentativeCost = dist.get(currentKey) + TILE_COSTS[world[next.row][next.col]];
...
function aStar(start, end) {
  ...
  while (openSet.length > 0) {
    ...
    for (const next of neighbors(current)) {
      ...
      const tentativeG = dist.get(currentKey) + TILE_COSTS[world[next.row][next.col]];
...
function animateResult(result) {
  ...
  isAnimating = true;
  setPathfindingButtonsDisabled(true);
  render();
  if (startCell) drawCellOutline(startCell.row, startCell.col, "#22aa22", 4);
  if (endCell) drawCellOutline(endCell.row, endCell.col, "#cc2222", 4);
  ...
  animationInterval = setInterval(() => {
    if (step < result.explored.length) {
      const cell = result.explored[step];
      const x = cameraX + (cell.col * TILE_SIZE);
      const y = cameraY + (cell.row * TILE_SIZE);
      ctx.fillStyle = "rgba(0, 255, 255, 0.5)";
      ctx.fillRect(x, y, TILE_SIZE, TILE_SIZE);
      ...
      if (result.path) {
        ...
        showPopup(`Path found! Explored ${result.nodesExplored} nodes.`);
      } else {
...
function setPathfindingButtonsDisabled(disabled) {
  document.getElementById("astar-btn").disabled = disabled;
  document.getElementById("bfs-btn").disabled = disabled;
  document.getElementById("dijkstra-btn").disabled = disabled;
}

function drawCellOutline(row, col, color, thickness) {
  const x = cameraX + col * TILE_SIZE;
  const y = cameraY + row * TILE_SIZE;
